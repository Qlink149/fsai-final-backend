...
from qlink_chatbot.prompt.general_prompt import general_prompt
from qlink_chatbot.utils.get_openai_responses import get_openai_responses
from qlink_chatbot.utils.logger_config import logger


class SearchAgent(Processor):
    """Search a query."""
    ...
    async def process(self, data: dict) -> dict:
        ...
        try:
            if "text" in data["messages"]:
                ...
                user_message = "please tell me about the FSAI and event."

            logger.info(
                "Request received to search query", extra={"query": user_message}
            )
            ...
            logger.exception(
                "Exception occured while running search agent.",
